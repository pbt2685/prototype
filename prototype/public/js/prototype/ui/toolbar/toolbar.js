	...
	setup_help() {
		...
		$(document).on("page-change", function () {
			...
			if (route_str.includes("/")) {
				...
				if (route_str.startsWith("query-report/")) {
					$(".navbar-title")
						.text(__(doctype_text))
						.attr("href", "/app/query-report/" + doctype_text);
				} else {
				...
				doctype_text = doctype_text
					...
					.join(' ');
				$(".navbar-title").text(__(doctype_text));
			}
			

			let current_workspace_name = frappe.get_route_str().split('/')[1] || '';

			// Chọn tất cả block workspace
			document.querySelectorAll('.workspace-block').forEach(block => {
				// Lấy slug của workspace từ href
				let workspace_slug = block.getAttribute('href').split('/app/')[1];

				// Nếu trùng route hiện tại → active
				if(workspace_slug === current_workspace_name.toLowerCase()) {
...
let all_workspaces = frappe.workspaces;
let container = document.querySelector('.workspace-navbar .container');

// Tạo wrapper
let wrapper = document.createElement('div');
wrapper.className = 'workspace-wrapper d-flex flex-nowrap';
wrapper.style.overflowX = 'auto';
wrapper.style.overflowY = 'hidden';
wrapper.style.scrollbarWidth = 'none'; // Firefox
wrapper.style.msOverflowStyle = 'none'; // IE 10+
wrapper.style.whiteSpace = 'nowrap'; // cho inline-block nếu muốn

// Ẩn scrollbar cho Webkit
let style = document.createElement('style');
document.head.appendChild(style);

// Tạo các block workspace
Object.values(all_workspaces).filter(ws => !ws.is_hidden).forEach(ws => {
    let slug = ws.name.toLowerCase()
                     .replace(/\s+/g, '-')
                     .replace(/[^a-z0-9\-]/g, '');

    let link = document.createElement('a');
    link.href = `/app/${slug}`;
    link.className = 'workspace-block text-center px-4 py-2 text-decoration-none text-dark';
    link.style.flex = '0 0 auto'; // width = nội dung
    link.textContent = __(ws.title) || __(ws.name);
